@if(count($records))
    @foreach($records as $section)
        <tr id="tableRecord-{{$section->id}}">
            ...
            @endif
            <td>{{$section->PhotoSectionType->name}}</td>
            ...
            <td>
                <a
                    href="{{route('admins.photo.edit.section',$section->id)}}"
                    title="Edit Section"
                ...
                <a
                    ...
                    onclick="injectModalData('{{$section->id}}', '{{route('admins.photo.destroy.section', $section->id)}}', 'confirm-password-form', 'DELETE')"
                >
