<form  method="GET"  action="{{route('admins.photo.export')}}" id="filterDataForm">
    <div style="display: flex;justify-content: flex-end;align-items:flex-end;margin-bottom: 20px">
        <div class="input-group" style="width: 40%">
            <select
                name="category"
                id="photoCategoryColumn"
                class="form-control"
                data-live-search="true"
                title="Please select photo category ...">
                <option value="">-- Please select photo category --</option>
                @foreach($photo_categories as $photo_category)
                    <option value="{{$photo_category->id}}">{{$photo_category->name}}</option>
                @endforeach
            </select>
        </div>
