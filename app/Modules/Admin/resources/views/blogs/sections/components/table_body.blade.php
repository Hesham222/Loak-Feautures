@if(count($records))
    @foreach($records as $section)
        <tr id="tableRecord-{{$section->id}}">
            ...
            @endif
            <td>{{$section->BlogSectionType->name}}</td>
            ...
            <td>
                <a
                    href="{{route('admins.blog.edit.section',$section->id)}}"
                    title="Edit Section"
                ...
                <a
                    ...
                    onclick="injectModalData('{{$section->id}}', '{{route('admins.blog.destroy.section', $section->id)}}', 'confirm-password-form', 'DELETE')"
                >
