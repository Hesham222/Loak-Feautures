...
class SectionUpdateAction
{
    ...
    {
        $record                         = ProjectSection::find($id);

        if (!ProjectSection::where(['project_id'=> $record->project_id,'order'=>$request->input('order')])->exists()){
            ...
            foreach ($record->SectionValues as $sectionValue)
            {
                ...
                if (!is_null($file))
                {
                    ...
                    $newFile = FileTrait::storeSingleFile($file,'project_sections');
